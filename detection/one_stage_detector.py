...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        c3Shape = dummy_out_shapes[0][1]
        c4Shape = dummy_out_shapes[1][1]
        c5Shape = dummy_out_shapes[2][1]

        c3conv1 = nn.Conv2d(c3Shape[1],self.out_channels,kernel_size=1)
        c4conv1 = nn.Conv2d(c4Shape[1],self.out_channels,kernel_size=1)
        c5conv1 = nn.Conv2d(c5Shape[1],self.out_channels,kernel_size=1)

        c3Convout = nn.Conv2d(self.out_channels,self.out_channels,3,stride=1, padding = 1)
        c4Convout = nn.Conv2d(self.out_channels,self.out_channels,3,stride=1, padding = 1)
        c5Convout = nn.Conv2d(self.out_channels,self.out_channels,3,stride=1, padding = 1)

        self.fpn_params = nn.ModuleDict({
            "c3": nn.ModuleList([c3conv1, c3Convout]),
            "c4": nn.ModuleList([c4conv1, c4Convout]),
            "c5": nn.ModuleList([c5conv1, c5Convout])
        })
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats["p5"] = self.fpn_params["c5"][1](self.fpn_params["c5"][0](backbone_feats["c5"]))
        fpn_feats["p4"] = self.fpn_params["c4"][1](self.fpn_params["c4"][0](backbone_feats["c4"])) + F.interpolate(fpn_feats["p5"],scale_factor=2)
        fpn_feats["p3"] = self.fpn_params["c3"][1](self.fpn_params["c3"][0](backbone_feats["c3"])) + F.interpolate(fpn_feats["p4"],scale_factor=2)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        boxreg_deltas = {}
        centerness_logits = {}
        BatchSize = feats_per_fpn_level["p3"].size(dim = 0)
        for i, key in enumerate(feats_per_fpn_level.keys()):
            stem1 = self.stem_cls(feats_per_fpn_level[key])
            stem2 = self.stem_box(feats_per_fpn_level[key])
            class_logits[key] = self.pred_cls(stem1).view(BatchSize,-1,self.num_classes)
            boxreg_deltas[key] = self.pred_box(stem2).view(BatchSize,-1,4)
            centerness_logits[key] = self.pred_ctr(stem2).view(BatchSize,-1,1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        self.strides_per_fpn = self.backbone.fpn_strides
...
# )     
        ...
        fpn_feats_shapes = {"p3": (B,C,28,28), "p4":(B,C,14,14), "p5": (B,C,7,7)}
        strides_per_fpn_level = {"p3": 8, "p4": 16, "p5": 32}
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats_shapes, strides_per_fpn_level, device ="cpu")
...
# )
        for i in range(B):
            boxes = gt_boxes[i]
            matched_loc = fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_level, boxes)
    ...
    # locations: torch.Tensor, gt_boxes: torch.Tensor, stride: int)
        ...
        for i in range(B):
            delta_dict = {}
            for key in strides_per_fpn_level.keys():
                gt_deltas = fcos_get_deltas_from_locations(locations_per_fpn_level[key], matched_gt_boxes[i][key], strides_per_fpn_level[key])
                delta_dict[key] = gt_deltas
        ...
        N = matched_gt_boxes.size(dim = 1)
        gt_cls = torch.zeros(B,N,self.num_classes)
        for i in range(B):
            for j in range(N):
                idx = int(matched_gt_boxes[i,j,4].item())
                if (idx != -1):
                    # print(idx)
                    gt_cls[i,j,idx] = 1

        loss_cls = sigmoid_focal_loss(inputs = pred_cls_logits.to("cuda"),targets = gt_cls.to("cuda"))
        ...
        for i in range(B):
            gt_batch_ctr = fcos_make_centerness_targets(matched_gt_deltas[i])
            # print(gt_ctr[i].size())
            # print(gt_batch_ctr.size())
            gt_ctr[i][:,0] = gt_batch_ctr
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Replace "pass" statement with your code
            for i in range(N):
                #print(level_pred_scores[i])
                level_pred_classes[i] = torch.argmax(level_pred_score[i]).item()
                level_pred_scores[i] = torch.max(level_pred_score[i]).item()
            
            # Step 2:
            # Replace "pass" statement with your code
            level_deltas = level_deltas[level_pred_scores > test_score_thresh]
            level_locations = level_locations[level_pred_scores > test_score_thresh]
            ...
            for i in range(len(level_pred_boxes)):
                ...
                level_pred_boxes[i,2] = torch.clamp(level_pred_boxes[i,2], max = W-1)
                level_pred_boxes[i,3] = torch.clamp(level_pred_boxes[i,3], max = H-1)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
